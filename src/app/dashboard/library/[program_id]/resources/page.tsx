    ...
    return (
        <section className="w-full grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
            ...
            { foundSection.resources.map( (resource, i:number) =>
                ...
                : resource.link &&
                    <Link key={i} href={resource.link} target="_blank" className="w-full p-4 lg:p-6 rounded-lg bg-card border border-card-light card-link flex items-center gap-4 lg:gap-6">
                        <div className="size-12 shrink-0">
                            <ResourceIcon type={resource.type} />
                        </div>
                        <div className="flex-1 min-w-0">

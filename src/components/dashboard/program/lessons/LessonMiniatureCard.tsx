...
export const LessonMiniatureCard = ({ lesson, index, programId, isCurrent, isCompleted, isAvailable }: Props) => {
    ...
    const Thumbnail = ({ overlay }: { overlay?: React.ReactNode }) => (
        <div className="col-span-2 aspect-video rounded-lg relative overflow-hidden bg-card-light flex items-center justify-center">
            {overlay && <div className="size-full bg-black/75 flex justify-center items-center absolute top-0 left-0 z-10">{overlay}</div>}
    ...
    if(isAvailable) {
        return (
            ...
            >
                <Thumbnail overlay={<PlayIcon className="size-8 opacity-0 group-hover:opacity-100 transition-200" />} />
        ...
        return (
            <div className="w-full grid grid-cols-5 p-2 rounded-lg opacity-50">
                <Thumbnail overlay={<LockIcon className="size-8 text-card-lightest" />} />
                <div className="col-span-3 px-4 py-2 flex flex-col">
                    <p className="subtitle-1">Lección {index < 10 ? `0${index}` : index}</p>
                    <h2 className="w-full title-3 grow truncate">{lesson.title}</h2>

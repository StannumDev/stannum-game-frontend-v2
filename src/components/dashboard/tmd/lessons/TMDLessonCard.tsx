...
export const TMDLessonCard = ({index, title, completed}:Props) => {
    return (
        <Link href={'/dashboard/library/tmd/lessons'} className="w-full h-20 flex items-center bg-card/25 hover:bg-card-light/40 rounded-lg relative overflow-hidden group cursor-pointer transition-200">
            <div className={`w-20 h-full ${ completed ? 'bg-stannum' : 'bg-card' } flex justify-center items-center shrink-0 ${styles.index__clip__diagonal}`}>
            ...
            { completed ?
                ...
                <div className='w-fit flex items-center gap-4 shrink-0 relative z-10'>
                    <div className='subtitle-1'>Pendiente</div>

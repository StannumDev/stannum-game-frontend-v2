...
export const TMDInstructionCard = ({index, title, inProcess, completed, handleInstructionChange }:Props) => {
    return (
        <button
            ...
            className={`w-full h-52 flex text-start items-center bg-card/25 hover:bg-card-light/40 rounded-lg border-2 ${ completed ? 'border-stannum' : 'border-card hover:border-card-light' } relative overflow-hidden group cursor-pointer transition-200 `}
        >
